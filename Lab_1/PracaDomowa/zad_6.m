...
any(A(:)<0)
disp('4. Znajdz wspolrzedne elementu o wartosci maksymalnej (numer wiersza i numer kolumny).')
[ floor(find(A==max(A(:))) / 10), rem(  find(A==max(A(:))),  10) ]
disp('5. Wypisz macierz z elementami macierzy A z przedzialu (-0.25,0.25). Pozostale miejsca maja byc wypelnione zerami.')
(abs(A)<0.25) .* A
